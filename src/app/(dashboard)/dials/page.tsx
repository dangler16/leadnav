...
import { DialsFilterTabs } from './dials-filter-tabs'
...
type LeadForCall = Pick<Lead, 'id' | 'firstname' | 'lastname' | 'phone'>

function sortDials(data: DialWithLead[], col: string | null, dir: SortDir | null): DialWithLead[] {
  ...
  return [...data].sort((a, b) => {
    ...
    else if (col === 'ended_by') { av = a.ended_by ?? null; bv = b.ended_by ?? null }
...
export default async function DialsPage({
  ...
  let leadsForCall: LeadForCall[] = []

  if (role === 'super_admin') {
    const [{ data: dialData }, { data: leadsData }] = await Promise.all([
      supabase.from('call_logs').select('*, leads(id, firstname, lastname)').order('called_at', { ascending: false }),
      supabase.from('leads').select('id, firstname, lastname, phone').order('firstname'),
    ])
    dials = (dialData ?? []) as DialWithLead[]
    leadsForCall = (leadsData ?? []) as LeadForCall[]

    const agentIds = [...new Set(dials.map(c => c.agent_id))]
    if (agentIds.length > 0) {
      ...
      dials = dials.map(c => ({ ...c, agentProfile: profileById[c.agent_id] ?? null }))
    }
    ...
    if (myMemberIds.length > 0) {
      const [{ data: dialData }, { data: leadsData }] = await Promise.all([
        ...
        supabase.from('leads').select('id, firstname, lastname, phone').in('assigned_to', myMemberIds).order('firstname'),
      ])
      dials = (dialData ?? []) as DialWithLead[]
      leadsForCall = (leadsData ?? []) as LeadForCall[]
    ...
    const [{ data: dialData }, { data: leadsData }] = await Promise.all([
      ...
      supabase.from('leads').select('id, firstname, lastname, phone').eq('assigned_to', user.id).order('firstname'),
    ])
    dials = (dialData ?? []) as DialWithLead[]
    leadsForCall = (leadsData ?? []) as LeadForCall[]
  }

  const filtered = sortDials(
    dials.filter(c => {
  ...
  const counts: Record<FilterTab, number> = {
    All: dials.length,
    Contacted: dials.filter(c => c.outcome === 'contacted').length,
    'Appointment Set': dials.filter(c => c.outcome === 'appointment_set').length,
    Sale: dials.filter(c => c.outcome === 'sale').length,
    'No Answer': dials.filter(c => c.outcome === 'no_answer' || c.outcome === 'voicemail').length,
  }
    ...
    <div className="flex flex-col h-full overflow-hidden bg-background">
      ...
      <div className="flex flex-col flex-1 min-h-0 mx-8 mb-5 border border-border rounded-lg overflow-hidden">
        <div className="px-3 py-2.5 border-b border-border shrink-0">
          <DialsFilterTabs filter={filter} counts={counts} />
        </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-border">
                ...
                <th className="text-left px-3 py-2.5"><SortableHeader column="ended_by" label="Ended By"  currentSort={sort} currentDir={sortDir} /></th>
                <th className="text-left px-3 py-2.5 text-xs font-medium text-muted-foreground">Recording</th>
            ...
            <tbody>
              {filtered.length === 0 && (
                <tr>
                  <td colSpan={colCount} className="py-12 text-center text-xs text-muted-foreground">No dials found</td>
                </tr>
              )}
              {filtered.map(dial => (
                <tr key={dial.id} className="border-b border-border hover:bg-muted transition-colors">
                  <td className="px-3 py-2.5">
                    {dial.leads ? (
                      <Link href={`/leads/${dial.leads.id}`} className="text-xs font-medium text-foreground hover:text-muted-foreground transition-colors">
                        {[dial.leads.firstname, dial.leads.lastname].filter(Boolean).join(' ') || '—'}
                      </Link>
                  ...
                  {isAdmin && (
                    <td className="px-3 py-2.5 text-xs text-foreground">
                      {dial.agentProfile
                        ? [dial.agentProfile.first_name, dial.agentProfile.last_name].filter(Boolean).join(' ')
                        : <span className="text-muted-foreground">—</span>}
                    </td>
                  )}
                  <td className="px-3 py-2.5">
                    <span className={cn(badgeShape, outcomeColors[dial.outcome])}>
                      {outcomeLabels[dial.outcome]}
                    </span>
                  </td>
                  <td className="px-3 py-2.5 text-xs text-muted-foreground tabular-nums">{formatDuration(dial.duration_seconds)}</td>
                  <td className="px-3 py-2.5 text-xs text-muted-foreground whitespace-nowrap">
                    {dial.ended_by === 'agent' ? 'Agent' : dial.ended_by === 'lead' ? 'Lead' : '—'}
                  </td>
                  <td className="px-3 py-2.5">
                    {dial.recording_url ? (
                      <a
                        href={dial.recording_url}
                        target="_blank"
                  ...
                  <td className="px-3 py-2.5 text-xs text-muted-foreground whitespace-nowrap">{formatDate(dial.called_at)}</td>
                </tr>
              ))}
            </tbody>
          </table>
