          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-border">
                ...
                <th className="text-left text-xs font-medium uppercase tracking-wide text-muted-foreground px-3 py-2.5">Lead Types</th>
                <th className="text-left text-xs font-medium uppercase tracking-wide text-muted-foreground px-3 py-2.5">Locations</th>
            ...
            <tbody>
              {sorted.length === 0 && (
                <tr>
                  <td colSpan={7} className="px-3 py-12 text-center text-xs text-muted-foreground">No vendors yet.</td>
                </tr>
              )}
              {sorted.map(v => {
                  ...
                  <tr key={v.id} className="border-b border-border hover:bg-muted transition-colors">
                    <td className="px-3 py-2.5">
                      <div className="flex items-center gap-2.5">
                        {v.logo_url ? (
                          <img src={v.logo_url} alt={v.name} className="w-7 h-7 rounded object-cover border border-border shrink-0" />
                        ) : (
